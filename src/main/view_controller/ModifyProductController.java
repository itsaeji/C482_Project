...
import static main.view_controller.MainController.getIndexOfProductToModify;
...
public class ModifyProductController implements Initializable
{
    ...
    public void initialize(URL url, ResourceBundle resourceBundle)
    {
        ...
        updateAllPartsTable();
        int indexOfProduct = getIndexOfProductToModify();
        Product productToModify = Inventory.lookupProduct(indexOfProduct);
        txtId.setText(Integer.toString(productToModify.getID()));
        txtInvLvl.setText(Integer.toString(productToModify.getStock()));
        ...
        txtPPU.setText(Double.toString(productToModify.getPrice()));

        if(productToModify.getAllAssociatedParts().size() != 0)
        {
            asParts = productToModify.getAllAssociatedParts();
            updateAsPartsTable();
        }
    }
    ...
    @FXML
    public void modifyProductSave(ActionEvent actionEvent) throws Exception
    {
        ...
        if(asParts.size() != 0)
        {
            ...
            {
                Inventory.updateProduct(getIndexOfProductToModify(), newProduct);
